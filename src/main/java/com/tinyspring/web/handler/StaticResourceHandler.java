...
public class StaticResourceHandler implements IHandler {
	...
	public void handle(HttpServletRequest request, HttpServletResponse response) throws HandlerException {
		System.out.println("Static handling: " + request.getRequestURI());
		if (!request.getRequestURI().startsWith("/resources/")) {
			throw new HandlerException("Not a static resource");
		}
	...
	protected String getPath(HttpServletRequest req) {
		...
		String pathInfo = StaticResourceHandler.coalesce(req.getPathInfo(), "");
	...
	protected String getMimeType(String path) {
		return StaticResourceHandler.coalesce(this.servletContext.getMimeType(path),"application/octet-stream");
	}
